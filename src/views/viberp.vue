<template>
  <div class="mx-auto w-full md:w-4/5 lg:w-3/4 2xl:w-1/2">
    ...
    <input
      class="mt-2 md:mb-t4 bg-gray-800 rounded p-t1 w-full sm:w-3/4 xl:w-4/5"
      v-model="url"
      v-on:keyup.enter="getSpotifyData"
      placeholder="Wklej link playlisty lub piosenki ..."
    />

    <div
      class="inline-flex items-center justify-center px-4 py-3 shadow-lg text-base font-medium leading-6 transition duration-300 bg-202020 hover:bg-282828 rounded-md cursor-pointer mx-t0 mt-t1 mb-t3 sm:my-t4 sm:mx-t2"
      @click="getSpotifyData()"
    >
      Wyszukaj
    </div>

    <div class="grid grid-cols-1 xl:w-4/5 2xl:w-full 2xl:grid-cols-2 gap-4">
      <div v-for="sitem in spotifyData" :key="sitem.id">
        <div
          class="bg-202020 hover:bg-282828 grid grid-cols-1 sm:grid-cols-1f3f gap-6 sm:gap-8 anim-fade-in transition-all duration-300 p-5 rounded-sm"
          v-if="sitem.method == 'api'"
        >
          <div class="grid grid-cols-1f3fa sm:block pb-2">
            <div
              class="relative rounded-sm shadow-2xl overflow-hidden transition duration-300 transform hover:scale-105"
            >
              <a :href="sitem.url" class="h-auto">
                <img
                  :src="sitem.data.images[0].url || sitem.data.images.url"
                  class="w-full h-auto"
                />
              </a>
            </div>
            <div class="px-t2 sm:px-0 sm:pt-t2">
              <h2 class="pt-0 font-semibold tracking-tight">
                {{ sitem.data.name }}
              </h2>
              <p
                class="text-gray-500 text-sm sm:text-base tracking-tighter"
                v-if="sitem.type == 'playlist'"
              >
                {{ sitem.data.owner.display_name }}<br />{{
                  sitem.data.tracks.total
                }}
                songs
              </p>
              <p
                ...
                v-else
              >
                {{ formatArtists(sitem.data.artists) }}
              </p>
            </div>
            <a
              :href="sitem.url"
              class="inline-block sm:my-3"
              title="Play on Spotify"
            >
              <div
                class="bg-spotifygreen hover:bg-spotifygreen-darker transition-all duration-300 transform hover:scale-105 rounded-full font-semibold p-t1 xs:px-t2 xs:py-t05 shadow-lg hover:shadow-xl"
              >
                <svg
                  class="inline-block"
                ...
                <span class="hidden xs:inline-block text-sm pl-t1 text-white"
                  >Play</span
                >
              </div>
            </a>
          ...
          <div class="grid grid-cols-1 content-between">
            <div
              class="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-2 gap-2"
              v-if="sitem.type == 'playlist'"
            >
              <div
                class="flex pb-t1 transition-all duration-300 transform hover:scale-105 cursor-pointer"
              ...
              >
                <div class="w-8 flex-none">
                  <svg
                    v-if="strack.track.preview_url == null"
                    class="m-1 mr-2 w-4 h-auto"
                    viewBox="0 0 24 24"
                  >
                    <path
                      fill="currentColor"
                  ...
                  <svg
                    v-else-if="
                      playingUrl == strack.track.preview_url &&
                      isPlaying == true
                    "
                    class="m-1 mr-2 w-4 h-auto"
                    viewBox="0 0 24 24"
                ...
                <div class="flex-grow overflow-hidden">
                  <p
                    class="text-sm xs:text-base p-0 mb-1 leading-5"
                    :title="strack.track.name"
                  >
                    {{ strack.track.name }}
                  </p>
                  <p
                    class="text-gray-500 text-xs xs:text-sm leading-4 p-0 mb-0"
                    :title="formatArtists(strack.track.artists)"
                  >
                    {{ formatArtists(strack.track.artists) }}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                <div class="w-8 flex-none">
                  ...
                  <svg
                    v-else-if="
                      playingUrl == strack.preview_url && isPlaying == true
                    "
                    class="m-1 mr-2 w-4 h-auto"
                    viewBox="0 0 24 24"
                  >
                ...
                <div class="flex-grow overflow-hidden">
                  ...
                  <p
                    class="text-gray-500 text-xs xs:text-sm leading-4 p-0 mb-0"
                    :title="formatArtists(strack.artists)"
                  >
                    {{ formatArtists(strack.artists) }}
                  </p>
                </div>
              </div>
            </div>
            <div class="w-full text-right pt-3 pb-0"></div>
          </div>
        </div>
        <div
          class="bg-202020 hover:bg-282828 gap-4 sm:gap-8 anim-fade-in transition-all duration-300 p-t2 rounded-sm"
          v-else
        >
          <div v-if="sitem.method == 'embed'">
            <iframe
              :src="sitem.data"
              class="w-full h-25r border-0 rounded-sm"
              :class="sitem.type == 'track' && 'max-h-64'"
              :title="'Spotify embed for ' + sitem.url"
              loading="lazy"
...
export default {
  ...
  async created() {
    ...
    if (
      ...
      this.$store.state.spotifyMethod === "api"
    ) {
      const responsePost = await fetch(
        "/.netlify/functions/getSpotifyToken"
      ).catch(function (err) {});
      if (!responsePost.ok) {
        this.$store.commit("spotifyMethod", "embed");
      }
  ...
  methods: {
    ...
    async getSpotifyData() {
      ...
      if (itemFormat == "track" && this.urlMethod == "api") {
        let resp = await fetch("/.netlify/functions/getSpotifyData", {
          method: "GET",
          headers: {
            itemformat: itemFormat,
            itemid: itemId,
            itemtoken: this.$store.state.access_token,
          },
        }).catch((err) => this.spotifyNetworkError(err));
        try {
          ...
          this.spotifyNetworkError("błąd sieci");
        }
